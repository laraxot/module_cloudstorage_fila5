# 📚 Indice Documentazione Modulo CloudStorage

**Status**: ✅ PHPStan Level 10 Compliant  
**Module Version**: 2.3.0

## 🎯 Lettura Essenziale

1. [README.md](./README.md) - Panoramica completa, Quick Start e stato reale del codice.
2. [roadmap/README.md](./roadmap/README.md) - Piano evolutivo e obiettivi.
3. [tasks/tasks-index.md](./tasks/tasks-index.md) - Indice delle task del modulo.

## 🏗️ Architettura

- **[Wiki Index](./wiki/index.md)** - Struttura e concetti chiave.
- **[Tasks](./tasks/tasks-index.md)** - Indice delle task del modulo.
- **[BMAD Method](./wiki/bmad-method.md)** - Applicazione del metodo BMAD.

## ☁️ Integrazione Google Drive

- **[gdrive/api.md](./gdrive/api.md)** - Riferimenti API Google Drive.
- **[gdrive/tutorial.md](./gdrive/tutorial.md)** - Tutorial di integrazione.
- **[gdrive/spatie.md](./gdrive/spatie.md)** - Integrazione con Spatie.

## 📊 Qualità e Testing

- [coverage.md](./coverage.md) - Report di copertura.
- [git-conflict-resolution-2026-07-31.md](./git-conflict-resolution-2026-07-31.md) - Risoluzione conflitti.

## 📦 Pacchetti Composer

- `google/apiclient` - API Google Drive.
- `spatie/laravel-queueable-action` - Pattern Actions.

## 🔗 Moduli Correlati

- [Xot](../../Xot/docs/00-index.md) - Base framework.
- [Media](../../Media/docs/00-index.md) - Gestione file.
- [Cms](../../Cms/docs/00-index.md) - Integrazione editor.

## 📜 Regola Operativa

Prima di modificare codice: ragionare, studiare i docs del modulo, aggiornare docs/rules/memory/skills.

- Riferimento globale: [Pre-Edit Docs-First Rule](../../../../bashscripts/ai/wiki/rules/pre-edit-docs-first.md)
- Memory: [Pre-Edit Docs-First Memory](../../../../bashscripts/ai/wiki/rules/pre-edit-docs-first.md)